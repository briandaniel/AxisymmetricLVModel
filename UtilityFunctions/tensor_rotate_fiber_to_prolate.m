function [ Trot ] = tensor_rotate_fiber_to_prolate( Q, T )
...
Trot(:,:,2,2) = T(:,:,2,2).*cpsi.^2 + T(:,:,2,3).*s2psi  ...
                    + T(:,:,3,3).*spsi.^2;
                
Trot(:,:,2,1) = Trot(:,:,1,2);

Trot(:,:,2,3) = T(:,:,2,3).*c2psi+ (T(:,:,3,3) - T(:,:,2,2)).*cpsi.*spsi;
                    
                
Trot(:,:,3,1) = Trot(:,:,1,3);
Trot(:,:,3,2) = Trot(:,:,2,3);
                
Trot(:,:,3,3) = T(:,:,2,2).*spsi.^2 + cpsi.*(T(:,:,3,3).*cpsi - 2*spsi.*T(:,:,3,2));
